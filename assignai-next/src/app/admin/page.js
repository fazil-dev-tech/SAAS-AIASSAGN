...
export default function FuturisticAdminPortal() {
  ...
  const COLORS = ['#ffffff', '#e2e8f0', '#94a3b8', '#475569', '#1e293b'];


  if (isLoading) return null;

  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', backgroundColor: '#050505', color: '#f8fafc', overflow: 'hidden', position: 'relative' }}>
      <style>{`
      ...
      <div style={{ position: 'absolute', top: '-10%', left: '-10%', width: '50vw', height: '50vw', background: 'radial-gradient(circle, rgba(255,255,255,0.03) 0%, rgba(0,0,0,0) 70%)', filter: 'blur(100px)', pointerEvents: 'none', zIndex: 1 }} />
      <div style={{ position: 'absolute', bottom: '-20%', right: '-10%', width: '60vw', height: '60vw', background: 'radial-gradient(circle, rgba(255,255,255,0.02) 0%, rgba(0,0,0,0) 70%)', filter: 'blur(120px)', pointerEvents: 'none', zIndex: 1 }} />
          ...
          >
            <TiltCard style={{ width: '100%', maxWidth: '440px', padding: '1rem' }}>
              <div style={{ 
                ...
                boxShadow: '0 40px 80px -20px rgba(0, 0, 0, 0.8), inset 0 1px 1px rgba(255,255,255,0.1)', 
                ...
                <motion.h1 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} style={{ fontSize: '2.5rem', fontFamily: '"Playfair Display", serif', fontWeight: 800, letterSpacing: '-0.5px', margin: '0 0 0.5rem', background: 'linear-gradient(135deg, #fff, #cbd5e1)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', textShadow: '0 2px 10px rgba(0,0,0,0.5)' }}>AssignAI</motion.h1>
                <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} style={{ color: '#ec4899', marginBottom: '3.5rem', fontSize: '0.9rem', fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase' }}>Admin Central</motion.p>
                ...
                <form onSubmit={handleLogin} style={{ textAlign: 'left' }}>
                  <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 }} style={{ marginBottom: '1.25rem', position: 'relative' }}>
                    <Users size={18} style={{ position: 'absolute', left: '16px', top: '50%', transform: 'translateY(-50%)', color: '#64748b', transition: 'color 0.3s' }} className="input-icon" />
                    <input 
                      type="email" placeholder="System ID / Email" value={authEmail} onChange={e => setAuthEmail(e.target.value)} required 
                      style={{ width: '100%', padding: '16px 16px 16px 44px', background: 'rgba(0,0,0,0.4)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '18px', color: '#fff', fontSize: '0.95rem', outline: 'none', transition: 'all 0.3s', boxShadow: 'inset 0 2px 6px rgba(0,0,0,0.3)', textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap' }} 
                      onFocus={(e) => {e.target.style.borderColor = '#ec4899'; e.target.style.background = 'rgba(0,0,0,0.6)'; e.target.style.boxShadow = '0 0 0 4px rgba(236, 72, 153, 0.15), inset 0 2px 6px rgba(0,0,0,0.3)'; e.target.previousSibling.style.color = '#ec4899';}} 
                      onBlur={(e) => {e.target.style.borderColor = 'rgba(255,255,255,0.06)'; e.target.style.background = 'rgba(0,0,0,0.4)'; e.target.style.boxShadow = 'inset 0 2px 6px rgba(0,0,0,0.3)'; e.target.previousSibling.style.color = '#64748b';}} 
                    />
                  </motion.div>
                  
                  <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6 }} style={{ marginBottom: '3rem', position: 'relative' }}>
                    <Lock size={18} style={{ position: 'absolute', left: '16px', top: '50%', transform: 'translateY(-50%)', color: '#64748b', transition: 'color 0.3s' }} className="input-icon" />
                    <input 
                  ...
                  <motion.button 
                    ...
                    whileHover={{ scale: 1.03, boxShadow: '0 15px 35px rgba(236, 72, 153, 0.4), inset 0 2px 5px rgba(255,255,255,0.3)' }}
                    whileTap={{ scale: 0.97 }}
                    type="submit" 
                    style={{ position: 'relative', overflow: 'hidden', width: '100%', padding: '18px', background: 'linear-gradient(135deg, #ec4899 0%, #8b5cf6 100%)', color: '#ffffff', border: 'none', borderRadius: '18px', fontSize: '1.15rem', fontWeight: 800, letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer', transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px' }}
                  >
                    <motion.div animate={{ x: ['-200%', '200%'] }} transition={{ duration: 3, repeat: Infinity, ease: 'linear' }} style={{ position: 'absolute', top: 0, left: 0, width: '50%', height: '100%', background: 'linear-gradient(90deg, transparent, rgba(255,255,255,0.4), transparent)', transform: 'skewX(-20deg)', pointerEvents: 'none' }} />
                    <span style={{ position: 'relative', zIndex: 1, textShadow: '0 2px 4px rgba(0,0,0,0.3)' }}>Authenticate</span> 
      ...
      {isAdminLoggedIn && (
        <motion.div className="admin-layout" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1 }} style={{ display: 'flex', flex: 1, overflow: 'hidden', zIndex: 10 }}>
          ...
          <div className="admin-sidebar glass-panel" style={{ display: 'flex', flexDirection: 'column', paddingTop: '2.5rem', zIndex: 20 }}>
            <div className="sidebar-logo" style={{ padding: '0 1.5rem', marginBottom: '2.5rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                <div style={{ width: '36px', height: '36px', borderRadius: '10px', background: 'linear-gradient(135deg, #ffffff, #a3a3a3)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 8px 20px rgba(255, 255, 255, 0.1), inset 0 1px 0 rgba(255,255,255,0.4)' }}>
            ...
            <div className="nav-links" style={{ flex: 1, padding: '0 1rem' }}>
              ...
              {[
                ...
                return (
                  <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: '0.75rem', width: '100%', padding: '0.85rem 1rem', background: isActive ? 'rgba(255,255,255,0.1)' : 'transparent', border: 'none', borderRadius: '12px', color: isActive ? '#fff' : '#888888', fontSize: '0.95rem', fontWeight: 600, cursor: 'pointer', transition: 'all 0.3s', marginBottom: '0.4rem', overflow: 'hidden' }}>
                    ...
                    <div style={{ zIndex: 1, color: isActive ? '#fff' : 'currentColor' }}>{tab.icon}</div>
                    <span style={{ zIndex: 1 }}>{tab.label}</span>
                  </button>
                )
              })}

              <button className="spotlight-btn" onClick={() => setIsSearchOpen(true)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', padding: '0.85rem 1rem', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '12px', color: '#888888', fontSize: '0.9rem', fontWeight: 500, cursor: 'pointer', transition: 'all 0.2s', marginTop: '1.5rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                  <Search size={16} /> Spotlight
                </div>
                <div style={{ background: 'rgba(255,255,255,0.1)', padding: '2px 8px', borderRadius: '6px', fontSize: '0.7rem', fontWeight: 700, color: '#fff' }}>⌘K</div>
              </button>
            </div>

            <div className="sidebar-bottom" style={{ padding: '1.5rem', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
              <button onClick={handleLogout} style={{ width: '100%', padding: '0.85rem', background: 'rgba(255, 255, 255, 0.05)', border: '1px solid rgba(255, 255, 255, 0.1)', color: '#fff', borderRadius: '12px', fontSize: '0.95rem', fontWeight: 600, cursor: 'pointer', transition: 'background 0.2s' }} onMouseOver={(e) => e.target.style.background = 'rgba(255, 255, 255, 0.1)'} onMouseOut={(e) => e.target.style.background = 'rgba(255, 255, 255, 0.05)'}>
                Terminate Session
              </button>
            </div>
          </div>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', position: 'relative' }}>
            ...
            {/* Topbar */}
            <div className="admin-topbar glass-panel" style={{ height: '72px', borderBottom: '1px solid rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', padding: '0 2.5rem', justifyContent: 'space-between', zIndex: 10, borderRadius: '0' }}>
              <h2 style={{ fontSize: '1.2rem', fontWeight: 700, color: '#fff', margin: 0, textShadow: '0 2px 10px rgba(255,255,255,0.2)' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
                <div style={{ fontSize: '0.85rem', color: '#888888', display: 'flex', alignItems: 'center', gap: '0.5rem', fontWeight: 500 }}>
                  <Clock size={14} />
                  {new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'short', day: 'numeric' })}
                </div>
                <button onClick={() => window.location.href = '/'} style={{ padding: '8px 16px', background: '#ffffff', border: 'none', borderRadius: '20px', color: '#000000', fontSize: '0.85rem', fontWeight: 800, cursor: 'pointer', transition: 'all 0.2s', boxShadow: '0 4px 12px rgba(255,255,255,0.2)' }} onMouseOver={(e) => {e.target.style.transform = 'translateY(-1px)'}} onMouseOut={(e) => {e.target.style.transform = 'translateY(0)'}}>
                  Launch Web App ↗
                </button>
              </div>
            </div>
                ...
                <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: "easeOut" }}>
                  ...
                  <div className="admin-bento-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: '1.5rem', marginBottom: '2.5rem' }}>
                    ...
                    <TiltCard className="bento-widget glass-panel" style={{ gridColumn: 'span 4', borderRadius: '24px', padding: '1.8rem', display: 'flex', flexDirection: 'column', height: '100%', position: 'relative', overflow: 'hidden' }}>
                      <div style={{ position: 'absolute', top: '-10%', right: '-10%', width: '100px', height: '100px', background: 'radial-gradient(circle, rgba(255,255,255,0.1) 0%, rgba(0,0,0,0) 70%)', filter: 'blur(20px)' }} />
                      <Activity color="#ffffff" size={24} style={{ marginBottom: '1rem' }} />
                      <h4 style={{ color: '#a3a3a3', fontSize: '0.9rem', fontWeight: 600, margin: '0 0 0.5rem' }}>Core Engine</h4>
                      <div style={{ fontSize: '2.2rem', fontWeight: 800, color: '#fff', marginBottom: '1rem', letterSpacing: '-1px' }}>Optimal</div>
                      <div style={{ marginTop: 'auto', display: 'flex', gap: '1rem' }}>
                        <div style={{ flex: 1, background: 'rgba(255,255,255,0.03)', padding: '0.8rem', borderRadius: '14px', border: '1px solid rgba(255,255,255,0.05)' }}>
                          <div style={{ fontSize: '0.65rem', color: '#888888', textTransform: 'uppercase', fontWeight: 700, letterSpacing: '1px' }}>Ping</div>
                          <div style={{ fontSize: '1rem', color: '#ffffff', fontWeight: 700 }}>{ping}</div>
                        </div>
                        <div style={{ flex: 1, background: 'rgba(255,255,255,0.03)', padding: '0.8rem', borderRadius: '14px', border: '1px solid rgba(255,255,255,0.05)' }}>
                          <div style={{ fontSize: '0.65rem', color: '#888888', textTransform: 'uppercase', fontWeight: 700, letterSpacing: '1px' }}>Uptime</div>
                          <div style={{ fontSize: '1rem', color: '#ffffff', fontWeight: 700 }}>{uptimeDays} Days</div>
                        </div>
                      </div>
                    </TiltCard>

                    <TiltCard className="bento-widget glass-panel" style={{ gridColumn: 'span 4', borderRadius: '24px', padding: '1.8rem', display: 'flex', flexDirection: 'column', height: '100%', position: 'relative', overflow: 'hidden' }}>
                      <h4 style={{ color: '#a3a3a3', fontSize: '0.9rem', fontWeight: 600, margin: '0 0 0.5rem', zIndex: 1 }}>Docs Compiled</h4>
                      ...
                      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: '65%', opacity: 0.8, zIndex: 0 }}>
                        <ResponsiveContainer width="100%" height="100%">
                          <AreaChart data={chartData}>
                            ...
                            <Area type="monotone" dataKey="reports" stroke="#ffffff" fillOpacity={1} fill="url(#colorReports)" strokeWidth={2} />
                          </AreaChart>
                        </ResponsiveContainer>
                      </div>
                    </TiltCard>
                    ...
                    <TiltCard className="bento-widget glass-panel" style={{ gridColumn: 'span 4', borderRadius: '24px', padding: '1.8rem', display: 'flex', flexDirection: 'column', height: '100%', position: 'relative', overflow: 'hidden' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem' }}>
                        ...
                        <h4 style={{ color: '#ffffff', fontSize: '0.9rem', fontWeight: 700, margin: 0, textTransform: 'uppercase', letterSpacing: '1px' }}>Tokens Rendered</h4>
                      </div>
                      <div style={{ fontSize: '3rem', fontWeight: 800, color: '#fff', marginBottom: '0.5rem', letterSpacing: '-1px', textShadow: '0 4px 20px rgba(255,255,255,0.2)' }}>{stats.words.toLocaleString()}</div>
                      <div style={{ fontSize: '0.85rem', color: '#a3a3a3', marginTop: 'auto', fontWeight: 500 }}>Cumulative AI output across network.</div>
                    </TiltCard>
                  </div>
                  ...
                  <div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.2rem' }}>
                      ...
                      <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                        ...
                        <div style={{ display: 'flex', background: 'rgba(0,0,0,0.3)', borderRadius: '8px', padding: '4px' }}>
                          <button onClick={() => setReportViewArrangement('list')} style={{ background: reportViewArrangement === 'list' ? 'rgba(59, 130, 246, 0.2)' : 'transparent', color: reportViewArrangement === 'list' ? '#3b82f6' : '#64748b', border: 'none', padding: '6px 10px', borderRadius: '6px', cursor: 'pointer', transition: 'all 0.2s', display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.8rem', fontWeight: 600 }}>List</button>
                          <button onClick={() => setReportViewArrangement('grid')} style={{ background: reportViewArrangement === 'grid' ? 'rgba(59, 130, 246, 0.2)' : 'transparent', color: reportViewArrangement === 'grid' ? '#3b82f6' : '#64748b', border: 'none', padding: '6px 10px', borderRadius: '6px', cursor: 'pointer', transition: 'all 0.2s', display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.8rem', fontWeight: 600 }}>Grid</button>
                        </div>
                              ...
                              <tr key={r.id || i} style={{ borderTop: '1px solid rgba(255,255,255,0.05)', transition: 'background 0.2s', ':hover': { background: 'rgba(255,255,255,0.02)' } }}>
                                <td style={{ padding: '1.2rem 1.5rem' }}>
                                  <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                                    <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: 'rgba(255, 255, 255, 0.1)', border: '1px solid rgba(255, 255, 255, 0.2)', color: '#ffffff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.85rem', fontWeight: 800 }}>
                                      {r.user_id?.charAt(0).toUpperCase()}
                                    </div>
                                    <span style={{ fontSize: '0.95rem', color: '#ffffff', fontWeight: 600 }}>{r.user_id}</span>
                                  </div>
                                </td>
                                ...
                                <td style={{ padding: '1.2rem 1.5rem', textAlign: 'center' }}>
                                  <button onClick={() => setViewingReport(r)} style={{ background: 'transparent', border: 'none', color: '#ffffff', cursor: 'pointer', padding: '5px', transition: 'transform 0.2s' }} onMouseOver={e=>e.currentTarget.style.transform='scale(1.2)'} onMouseOut={e=>e.currentTarget.style.transform='scale(1)'}>
                                    <Eye size={18} />
                                  </button>
                                  <button onClick={() => deleteReport(r.id)} style={{ background: 'transparent', border: 'none', color: '#a3a3a3', cursor: 'pointer', padding: '5px', marginLeft: '10px', transition: 'transform 0.2s' }} onMouseOver={e=>e.currentTarget.style.transform='scale(1.2)'} onMouseOut={e=>e.currentTarget.style.transform='scale(1)'}>
                                    <Trash2 size={18} />
                                  </button>
                                </td>
                              </tr>
                          ...
                          <motion.div key={r.id || i} className="glass-panel" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.3, delay: i * 0.05 }} style={{ borderRadius: '20px', padding: '1.5rem', display: 'flex', flexDirection: 'column' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
                                <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: 'rgba(59, 130, 246, 0.15)', color: '#3b82f6', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.8rem', fontWeight: 800 }}>
                                  {r.user_id?.charAt(0).toUpperCase()}
                                </div>
                                <div style={{ fontSize: '0.8rem', color: '#94a3b8', maxWidth: '150px', overflow: 'hidden', textOverflow: 'ellipsis' }}>{r.user_id}</div>
                              </div>
                              <div style={{ fontSize: '0.7rem', color: '#64748b', background: 'rgba(0,0,0,0.3)', padding: '4px 8px', borderRadius: '12px' }}>
                                {new Date(r.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                              </div>
                            </div>
                            <h4 style={{ fontSize: '1rem', color: '#fff', margin: '0 0 0.5rem', lineHeight: 1.4 }}>
                            ...
                            <div style={{ fontSize: '1.2rem', fontWeight: 800, color: '#10b981', marginTop: 'auto', paddingTop: '1rem' }}>
                              +{r.word_count?.toLocaleString()} <span style={{ fontSize: '0.7rem', color: '#64748b', fontWeight: 600, textTransform: 'uppercase' }}>Tokens</span>
                            </div>
                            <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1rem', borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: '1rem' }}>
                              <button onClick={() => setViewingReport(r)} style={{ flex: 1, padding: '8px', background: 'rgba(59, 130, 246, 0.1)', border: 'none', color: '#3b82f6', borderRadius: '8px', cursor: 'pointer', fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', transition: 'background 0.2s' }} onMouseOver={e=>e.currentTarget.style.background='rgba(59, 130, 246, 0.2)'} onMouseOut={e=>e.currentTarget.style.background='rgba(59, 130, 246, 0.1)'}><Eye size={16}/> Inspect</button>
                              <button onClick={() => deleteReport(r.id)} style={{ flex: 1, padding: '8px', background: 'rgba(239, 68, 68, 0.1)', border: 'none', color: '#ef4444', borderRadius: '8px', cursor: 'pointer', fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', transition: 'background 0.2s' }} onMouseOver={e=>e.currentTarget.style.background='rgba(239, 68, 68, 0.2)'} onMouseOut={e=>e.currentTarget.style.background='rgba(239, 68, 68, 0.1)'}><Trash2 size={16}/> Purge</button>
                            </div>
                          </motion.div>
                        ))}
                      </div>
                    ...
                    {auditTotalPages > 1 && (
                      <div style={{ padding: '1rem', display: 'flex', justifyContent: 'center', gap: '1rem', marginTop: '1rem' }}>
                        <button disabled={auditPage === 1} onClick={() => setAuditPage(p => p - 1)} style={{ padding: '8px 20px', background: 'rgba(255,255,255,0.1)', border: 'none', borderRadius: '12px', color: '#fff', cursor: auditPage === 1 ? 'not-allowed' : 'pointer', opacity: auditPage === 1 ? 0.5 : 1, fontWeight: 600, transition: 'all 0.2s' }} onMouseOver={e=>!e.currentTarget.disabled && (e.currentTarget.style.background='rgba(255,255,255,0.2)')} onMouseOut={e=>!e.currentTarget.disabled && (e.currentTarget.style.background='rgba(255,255,255,0.1)')}>Previous</button>
                        <div style={{ color: '#94a3b8', fontSize: '0.9rem', display: 'flex', alignItems: 'center', background: 'rgba(0,0,0,0.3)', padding: '0 15px', borderRadius: '12px', fontWeight: 600 }}>Page {auditPage} of {auditTotalPages}</div>
                        <button disabled={auditPage === auditTotalPages} onClick={() => setAuditPage(p => p + 1)} style={{ padding: '8px 20px', background: 'rgba(255,255,255,0.1)', border: 'none', borderRadius: '12px', color: '#fff', cursor: auditPage === auditTotalPages ? 'not-allowed' : 'pointer', opacity: auditPage === auditTotalPages ? 0.5 : 1, fontWeight: 600, transition: 'all 0.2s' }} onMouseOver={e=>!e.currentTarget.disabled && (e.currentTarget.style.background='rgba(255,255,255,0.2)')} onMouseOut={e=>!e.currentTarget.disabled && (e.currentTarget.style.background='rgba(255,255,255,0.1)')}>Next</button>
                      </div>
                    )}
                  </div>
                </motion.div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: '1.5rem', marginBottom: '2.5rem' }}>
                    ...
                    <div className="glass-panel" style={{ gridColumn: 'span 7', borderRadius: '24px', padding: '1.5rem', display: 'flex', flexDirection: 'column' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                        <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                          <Users size={20} color="#ffffff" /> <span style={{ fontWeight: 600, fontSize: '1.1rem', color: '#fff' }}>Identity Growth (14 Days)</span>
                        </div>
                        <span style={{ color: '#000000', fontWeight: 800, background: '#ffffff', padding: '4px 10px', borderRadius: '12px' }}>Total: {dbUsers.length}</span>
                      </div>
                      <div style={{ height: '250px' }}>
                        <ResponsiveContainer width="100%" height="100%">
                          <AreaChart data={userGrowthData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                            ...
                            <XAxis dataKey="date" stroke="rgba(255,255,255,0.3)" fontSize={11} tickMargin={10} />
                            <YAxis stroke="rgba(255,255,255,0.3)" fontSize={11} />
                            <Tooltip contentStyle={{ background: 'rgba(255,255,255,0.05)', backdropFilter: 'blur(10px)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px', color: '#fff' }} itemStyle={{ color: '#fff' }} />
                            <Area type="monotone" dataKey="active_users" stroke="#ffffff" strokeWidth={4} fillOpacity={1} fill="url(#colorUsers)" />
                          </AreaChart>
                        </ResponsiveContainer>
                      </div>
                    </div>
                    ...
                    <div className="glass-panel" style={{ gridColumn: 'span 5', borderRadius: '24px', padding: '1.5rem', display: 'flex', flexDirection: 'column' }}>
                      <div style={{ display: 'flex', gap: '10px', alignItems: 'center', marginBottom: '1rem' }}>
                        <Database size={20} color="#ffffff" /> <span style={{ fontWeight: 600, fontSize: '1.1rem', color: '#fff' }}>Subject Intelligence</span>
                      </div>
                      ...
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', justifyContent: 'center' }}>
                        {subjectDistribution.map((entry, idx) => (
                          <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.75rem', color: '#cbd5e1' }}>
                    ...
                    <div className="admin-users-list glass-panel" style={{ borderRadius: '24px', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                      <div style={{ padding: '1.5rem 1.8rem', borderBottom: '1px solid rgba(255,255,255,0.05)', background: 'rgba(255,255,255,0.02)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <h3 style={{ margin: 0, fontSize: '1.2rem', fontWeight: 700, color: '#fff' }}>Identities</h3>
                          <select value={userSort} onChange={e => setUserSort(e.target.value)} style={{ background: 'rgba(255,255,255,0.05)', color: '#fff', border: '1px solid rgba(255,255,255,0.1)', padding: '4px 8px', borderRadius: '6px', fontSize: '0.75rem', outline: 'none' }}>
                      ...
                      <div style={{ flex: 1, overflowY: 'auto' }}>
                        {currentUsers.map((u, i) => (
                          <div key={i} onClick={() => setSelectedUser(u)} style={{ padding: '1.2rem 1.8rem', borderBottom: '1px solid rgba(255,255,255,0.02)', display: 'flex', alignItems: 'center', gap: '1.2rem', cursor: 'pointer', transition: 'all 0.2s', background: selectedUser?.email === u.email ? 'rgba(255, 255, 255, 0.05)' : 'transparent', borderLeft: selectedUser?.email === u.email ? '4px solid #ffffff' : '4px solid transparent' }}>
                            <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: u.is_suspended ? 'rgba(255, 255, 255, 0.02)' : 'linear-gradient(135deg, #ffffff, #a3a3a3)', boxShadow: u.is_suspended ? 'none' : '0 4px 10px rgba(255, 255, 255, 0.2)', color: u.is_suspended ? '#a3a3a3' : '#000000', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.2rem', fontWeight: 800 }}>
                              {u.name?.charAt(0).toUpperCase() || u.email.charAt(0).toUpperCase()}
                            </div>
                            <div style={{ flex: 1, overflow: 'hidden' }}>
                              <div style={{ fontSize: '1rem', fontWeight: 700, color: u.is_suspended ? '#555555' : '#ffffff', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', textDecoration: u.is_suspended ? 'line-through' : 'none' }}>{u.name || 'Unknown Agent'}</div>
                              <div style={{ fontSize: '0.8rem', color: '#a3a3a3', marginTop: '4px', fontWeight: 500 }}>{u.email}</div>
                            </div>
                            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
                              <div style={{ fontSize: '1.2rem', color: u.is_suspended ? '#555555' : '#ffffff', fontWeight: 800 }}>{u.reports}</div>
                            </div>
                          </div>
                        ))}
                      </div>
                      {usersTotalPages > 1 && (
                        <div style={{ padding: '0.8rem', display: 'flex', justifyContent: 'space-between', borderTop: '1px solid rgba(255,255,255,0.05)', background: 'rgba(255,255,255,0.02)' }}>
                          <button disabled={usersPage === 1} onClick={() => setUsersPage(p => p - 1)} style={{ padding: '5px 10px', background: 'transparent', border: 'none', color: '#ffffff', cursor: 'pointer' }}>&larr; Prev</button>
                          <span style={{ fontSize: '0.8rem', color: '#a3a3a3', alignSelf: 'center' }}>{usersPage} / {usersTotalPages}</span>
                          <button disabled={usersPage === usersTotalPages} onClick={() => setUsersPage(p => p + 1)} style={{ padding: '5px 10px', background: 'transparent', border: 'none', color: '#ffffff', cursor: 'pointer' }}>Next &rarr;</button>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="glass-panel" style={{ flex: 1, borderRadius: '24px', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                      {!selectedUser ? (
                        ...
                        <motion.div key={selectedUser.email} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} style={{ flex: 1, display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
                          {/* Profile Header */}
                          <div style={{ padding: '3rem', borderBottom: '1px solid rgba(255,255,255,0.05)', background: 'linear-gradient(180deg, rgba(255, 255, 255, 0.05) 0%, transparent 100%)', position: 'relative' }}>
                            {selectedUser.is_suspended && (
                              <div style={{ position: 'absolute', top: '1.5rem', right: '1.5rem', background: 'rgba(255, 255, 255, 0.1)', color: '#ffffff', padding: '5px 12px', borderRadius: '20px', fontSize: '0.75rem', fontWeight: 800, display: 'flex', alignItems: 'center', gap: '5px', border: '1px solid rgba(255, 255, 255, 0.3)' }}>
                                <ShieldAlert size={14} /> SUSPENDED
                              </div>
                            )}
                            <div style={{ display: 'flex', alignItems: 'center', gap: '2rem' }}>
                              <div style={{ width: '90px', height: '90px', borderRadius: '50%', background: selectedUser.is_suspended ? 'rgba(255, 255, 255, 0.05)' : 'linear-gradient(135deg, #ffffff, #a3a3a3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '2.5rem', fontWeight: 800, color: selectedUser.is_suspended ? '#a3a3a3' : '#000000', boxShadow: '0 10px 25px rgba(255,255,255,0.1), inset 0 2px 4px rgba(255,255,255,0.2)', border: selectedUser.is_suspended ? '1px solid rgba(255,255,255,0.2)' : 'none' }}>
                                {selectedUser.name?.charAt(0).toUpperCase() || selectedUser.email.charAt(0).toUpperCase()}
                              </div>
                              <div style={{ flex: 1 }}>
                                <h2 style={{ margin: '0 0 0.5rem', fontSize: '2rem', fontWeight: 800, color: selectedUser.is_suspended ? '#a3a3a3' : '#ffffff', letterSpacing: '-0.5px', textDecoration: selectedUser.is_suspended ? 'line-through' : 'none' }}>{selectedUser.name || 'Unknown User'}</h2>
                                <p style={{ margin: 0, color: '#a3a3a3', fontSize: '1rem', fontWeight: 500 }}>{selectedUser.email}</p>
                              </div>
                              <div>
                                <button onClick={() => impersonateUser(selectedUser)} style={{ padding: '10px 16px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '12px', color: '#fff', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', transition: 'background 0.2s' }} onMouseOver={e=>e.currentTarget.style.background='rgba(255,255,255,0.1)'} onMouseOut={e=>e.currentTarget.style.background='rgba(255,255,255,0.05)'}>
                          ...
                          {/* Action Buttons */}
                          <div style={{ padding: '1.5rem', display: 'flex', gap: '1rem', background: 'rgba(255,255,255,0.02)', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                            <button onClick={() => suspendUser(selectedUser.email, selectedUser.is_suspended)} style={{ flex: 1, padding: '12px', background: selectedUser.is_suspended ? 'rgba(255, 255, 255, 0.1)' : 'rgba(255, 255, 255, 0.05)', border: `1px solid ${selectedUser.is_suspended ? 'rgba(255, 255, 255, 0.4)' : 'rgba(255, 255, 255, 0.2)'}`, color: '#ffffff', borderRadius: '12px', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', transition: 'all 0.2s' }} onMouseOver={e=>e.currentTarget.style.background='rgba(255,255,255,0.15)'} onMouseOut={e=>e.currentTarget.style.background=selectedUser.is_suspended ? 'rgba(255, 255, 255, 0.1)' : 'rgba(255, 255, 255, 0.05)'}>
                              <PowerOff size={18} /> {selectedUser.is_suspended ? 'Restore Access' : 'Suspend Account'}
                            </button>
                            <button onClick={() => deleteUser(selectedUser.email)} style={{ flex: 1, padding: '12px', background: 'rgba(255, 0, 0, 0.1)', border: '1px solid rgba(255, 0, 0, 0.3)', color: '#ff4444', borderRadius: '12px', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', transition: 'all 0.2s' }} onMouseOver={e=>e.currentTarget.style.background='rgba(255, 0, 0, 0.2)'} onMouseOut={e=>e.currentTarget.style.background='rgba(255, 0, 0, 0.1)'}>
                          ...
                          <div style={{ padding: '2rem', flex: 1 }}>
                            ...
                            <div style={{ borderLeft: '2px solid rgba(255,255,255,0.1)', marginLeft: '10px', paddingLeft: '20px', position: 'relative' }}>
                              ...
                              <div style={{ marginBottom: '1.5rem', position: 'relative' }}>
                                ...
                                <div style={{ fontSize: '0.8rem', color: '#64748b', marginBottom: '2px' }}>{new Date(selectedUser.createdAt || new Date('2024-01-01')).toLocaleString()}</div>
                              ...
                              {savedReports.filter(r => r.user_id === selectedUser.email).map((r, idx) => (
                                <div key={idx} style={{ marginBottom: '1.5rem', position: 'relative' }}>
                                  ...
                                  <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '12px', padding: '1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    ...
                                    <button onClick={() => setViewingReport(r)} style={{ background: 'rgba(255, 255, 255, 0.05)', color: '#ffffff', border: '1px solid rgba(255, 255, 255, 0.2)', padding: '8px', borderRadius: '8px', cursor: 'pointer' }}>
                                      <Eye size={16} />
                                    </button>
                                  </div>
                                </div>
        ...
        {viewingReport && (
          ...
          >
            <motion.div 
              ...
              style={{ width: '100%', maxWidth: '1000px', height: '90vh', background: '#f8fafc', borderRadius: '16px', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 40px 80px rgba(0,0,0,0.8)' }}
            ...
            >
              ...
              <div style={{ padding: '1rem 2rem', background: '#0f172a', borderBottom: '1px solid rgba(255,255,255,0.1)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', zIndex: 10 }}>
                <div>
                  <h3 style={{ margin: 0, color: '#fff', fontSize: '1.1rem', fontWeight: 600 }}>{viewingReport.assignment_title || 'Report Preview'}</h3>
                  <div style={{ fontSize: '0.8rem', color: '#94a3b8', marginTop: '4px' }}>Generated by {viewingReport.user_id}</div>
                </div>
              ...
              <div className="report-page-container" style={{ flex: 1, background: '#94a3b8', padding: '2rem', borderRadius: '0', overflowY: 'auto', display: 'flex', justifyContent: 'center' }}>
      ...
      <style dangerouslySetInnerHTML={{__html: `
        ...
        @media screen and (max-width: 768px) {
          ...
          .admin-sidebar { width: 100% !important; padding-top: 1rem !important; border-right: none !important; border-bottom: 1px solid rgba(255,255,255,0.05); }
